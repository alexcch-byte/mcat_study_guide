...
> = {
  review: {
    label: "Review",
    text: "text-sky-300",
    chip: "bg-sky-500/10 ring-1 ring-inset ring-sky-500/25",
  ...
  content: {
    ...
    chip: "bg-violet-500/10 ring-1 ring-inset ring-violet-500/25",
  ...
  practice: {
    ...
    chip: "bg-emerald-500/10 ring-1 ring-inset ring-emerald-500/25",
  ...
  cars: {
    label: "CARS",
    text: "text-amber-300",
    chip: "bg-amber-500/10 ring-1 ring-inset ring-amber-500/25",
  ...
  full_length: {
    label: "Full-length",
    text: "text-rose-300",
    chip: "bg-rose-500/10 ring-1 ring-inset ring-rose-500/25",
  ...
  teaching: {
    ...
    chip: "bg-fuchsia-500/10 ring-1 ring-inset ring-fuchsia-500/25",
...
const FALLBACK_KIND = {
  ...
  chip: "bg-neutral-500/10 ring-1 ring-inset ring-neutral-500/25",
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-neutral-950 text-neutral-100">
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/4 -right-32 h-96 w-96 rounded-full bg-sky-500/15 blur-[100px]" />
        <div className="absolute bottom-0 left-1/3 h-72 w-72 rounded-full bg-emerald-500/10 blur-[100px]" />
      </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-1">
            ...
            <div className="flex flex-wrap gap-2 text-sm">
              <Link
                href="/diagnostics"
                className="px-3 py-1.5 rounded-full bg-neutral-900 ring-1 ring-inset ring-neutral-800 text-neutral-300 hover:text-white hover:ring-neutral-700 transition-colors"
              >
                Diagnostics
              </Link>
              <Link
          ...
          {totalMinutes > 0 && (
            <div className="mt-6 mb-8 p-4 rounded-2xl bg-neutral-900/60 ring-1 ring-inset ring-neutral-800">
              <div className="flex items-baseline justify-between mb-2">
                <span className="text-sm text-neutral-400">
                  {allDone ? (
                    <span className="text-emerald-400 font-medium">All done for today 🎉</span>
                  ) : (
                    <>
                      <span className="text-neutral-100 font-semibold">{doneMinutes}</span> / {totalMinutes} min done
                    </>
                  )}
                </span>
          ...
          {loading ? (
            <div className="space-y-3 animate-pulse">
              {[0, 1, 2].map((i) => (
                <div key={i} className="h-20 rounded-2xl bg-neutral-900/60 ring-1 ring-inset ring-neutral-800" />
              ))}
            </div>
          ) : blocks.length === 0 ? (
            <div className="text-center py-20 px-6 rounded-2xl bg-neutral-900/40 ring-1 ring-inset ring-neutral-800">
            ...
            <div className="space-y-3">
              {allDone && (
                <div className="p-4 rounded-2xl bg-emerald-500/10 ring-1 ring-inset ring-emerald-500/25 text-emerald-300 text-sm font-medium text-center">
                ...
                return (
                  <div
                    ...
                    className={`group relative overflow-hidden rounded-2xl ring-1 ring-inset transition-all ${
                      isDone
                        ? "bg-neutral-900/30 ring-neutral-900 opacity-60"
                        : "bg-neutral-900/70 ring-neutral-800 hover:ring-neutral-700 hover:bg-neutral-900 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20"
                    }`}
                  >
                    ...
                    <div className="flex items-start justify-between gap-4 p-4 pl-5">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          {isDone && <span className="text-xs text-emerald-500 font-medium">✓ done</span>}
                        </div>
                        {block.rationaleText && (
                          <p className="text-sm text-neutral-300 leading-snug">{block.rationaleText}</p>
                        )}
                      </div>
                      {!isDone && (
                        <Link
                          href={`/session?block=${block.id}`}
                          className="shrink-0 px-4 py-2 rounded-full bg-neutral-100 text-neutral-950 text-sm font-semibold hover:bg-white hover:scale-105 active:scale-95 transition-all"
                        >
                          Start
                        </Link>
                      )}
          ...
          <div className="mt-10 text-center">
            <button
              onClick={regenerate}
              className="text-xs text-neutral-600 hover:text-neutral-300 underline underline-offset-4 transition-colors"
            >
